(() => {
    ...
    const band_delay_promise = $.post(ajaxpath.banddel)
    ...
    const search_adv_promise = new Promise((r) => r([1,2,3,4,5])) ?? $.post(ajaxpath.adv);
    ...
    const top_advertisements = {
        advs: Random.shuffleArray([1, 2, 3, 4, 5]),
        current: 1
    };
    /**
     * 
    ...
    async function readyEventScheduleTable(type, miscvenue){
        ...
        BAND_SCHEDULE_DELAY = Number.isNaN(BAND_SCHEDULE_DELAY) ? (await band_delay_promise)["d"] : BAND_SCHEDULE_DELAY;
        ...
        const now = new Date();
        const delay = type == "band" ? BAND_SCHEDULE_DELAY : 0;
        var goes = null;
        var DAY = 1;
        ...
        for (var k = 0; k < searched_elements.length; k++){
            ...
            if (!nextdayelm){
                if (DAY == 1){
                    day2elements[0].elm.classList.add("now_playing_event", "metoscrkl");
        ...
        el.innerHTML = `
            <div class="event_scheduler_element">
                ...
                <div class="schedular_main_details">
                    <div class="taC">
                        <img class="Oiniums" src="/resources/cloud/event/${type}/${eventscheduler.name.replace(/ |:|\?/g, "_")}/icon.png"
                            onerror="this.remove()">
    ...
    function setTicketGUI(){
        ...
        $("#ticket_component > .afIOUW").remove();
